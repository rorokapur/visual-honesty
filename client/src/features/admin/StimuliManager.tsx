...
import { fetchStimuli, toggleSetEnabled } from "../../lib/admin";
...
export function StimuliManager() {
  ...
  const handleToggleSetEnabled = async (setId: string, enabled: boolean) => {
    try {
      await toggleSetEnabled(setId, enabled);
      setData((prev) =>
        prev.map((set) => (set.set_id === setId ? { ...set, enabled } : set)),
      );
    } catch (err: unknown) {
      const message = err instanceof Error ? err.message : String(err);
      alert("Failed to update set: " + message);
    }
  };
  ...
  useEffect(() => {
    const loadData = async () => {
      setLoading(true);
      try {
        const stimuli = await fetchStimuli();
        if (stimuli) {
          const grouped = (
            stimuli as (Stimulus & {
