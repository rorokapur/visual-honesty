drop function if exists "public"."add_stimulus"(p_image_url text, p_name text, p_is_deceptive boolean, p_set_name text);

drop view if exists "public"."pair_stats";

alter table "public"."sets" add column "category" text;

set check_function_bodies = off;

CREATE OR REPLACE FUNCTION public.add_stimulus(p_set_name text, p_image_url text, p_is_deceptive boolean, p_name text, p_category text)
 RETURNS void
 LANGUAGE plpgsql
AS $function$DECLARE
  v_set_id UUID;
